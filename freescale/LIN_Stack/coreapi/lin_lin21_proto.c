  ...
  )
{
  ...
  word_status_ptr->bit.overrun = word_status_ptr->bit.error_in_res | word_status_ptr->bit.successful_transfer;
  ...
  if (_MASTER_ == conf->function){
    ...
    if((LIN_FRM_EVNT == conf->frame_tbl[frame_index].frm_type) && (1 == etf_collision_flag)) 
    {
      word_status_ptr->bit.etf_collision = 1;
      word_status_ptr->bit.error_in_res = 0;
      lin_lld_clear_etf_collision_flag();      
    }
    else
