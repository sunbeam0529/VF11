...
/*--------------------------------------------------------------------*/
  ...
  pSCI->scicr1.byte = 0x00;    /* one start bit, eight data bits, one stop bit */
  pSCI->scicr2.byte = (SCICR2_TE_MASK | SCICR2_RE_MASK | SCICR2_RIE_MASK);
  pSCI->scisr2.byte |= (SCISR2_LBKDIF_MASK | SCISR2_BRK13_MASK | SCISR2_LBKDE_MASK);   /* clear LIN Break Detection flag       */
  pSCI->scicr3.byte |= SCICR3_FEIE_MASK;     /* Enable Frame Error interrupt         */
  pSCI->scibdh.byte |= SCIBDH_LBKDIE_MASK;        /* enable LIN Break Detection interrupt */
...
/*--------------------------------------------------------------------*/
  ...
  LPSLR = 0x01; /* Slew Rate for 9600 baudrate */
#endif /* END SCI0 */
  ...
  LP0SLRM = 0x01; /* Slew Rate for 9600 baudrate */ //这个位根本没写进去，写之前LP0CR_LPE=0才有效
#endif /* END SCI0 */
  ...
  /* Enable mismatch error detection */
  pSCI->sciasr1.byte = SCIASR1_BERRIF_MASK;
  /* Enable bit error interrupt and break detect flags */  //SCIACR1_RXEDGIE_MASK这个位是自己加的，lin唤醒stop mode使用的
  pSCI->sciacr1.byte = (SCIACR1_RXEDGIE_MASK | SCIACR1_BERRIE_MASK | SCIACR1_BKDIE_MASK);
  /* Enable break detect circuit and timing bit error detection at 9th time stick */
  pSCI->sciacr2.byte = (SCIACR2_BERRM0_MASK | SCIACR2_BKDFE_MASK);
  /* Enable tranceiver interrupt */
  pSCI->scicr2.byte  = (SCICR2_TE_MASK | SCICR2_RE_MASK | SCICR2_RIE_MASK);
...
#if (AUTOBAUD == 1)
void lin_lld_sci_init_autobaud
  (
		   l_u16 baudrate
  )
{
...
#if (defined(_MC9S12ZVL32_H))
 LP0CR_LPE = 1; /* Enable LIN PHY*/
 LP0SLRM = 0x01; /* Slew Rate for 9600 baudrate */
#endif /* END ifdef _MC9S12ZVL32_H */
 response_buffer=lin_lld_response_buffer;
 /* Set SCI is Master or Slave */
...
l_u8 Alive_RCCU_count=0;//子机的实时数据
l_u8 Alive_CCU_count=0; //主机的实时数据
  ...
  )
{
  ...
  *******************************/
  ...
  pSCI->sciasr1.byte |=0x80;//清除LIN Standby唤醒中断标志
  ...
  )
{
  ...
  /* Enable use of 13bit breaks and SCI frame for LIN */
  _pSCI->scicr1.byte = 0x00;    /* one start bit, eight data bits, one stop bit */
  _pSCI->scicr2.byte = (SCICR2_TE_MASK | SCICR2_RE_MASK);

  #if (SCI_VERSION == SCI_V2)
  _pSCI->scisr2.byte |= SCISR2_BRK13_MASK;   /* clear LIN Break Detection flag and set BRK13 bit       */
  _pSCI->scicr2.byte |= SCICR2_RIE_MASK;      /* enable RX complete interrupt         */
  _pSCI->scicr3.byte |= SCICR3_FEIE_MASK;     /* Enable Frame Error interrupt         */
  #endif /* End (SCI_VERSION == SCI_V2)*/
  ...
  _pSCI->scicr2.byte |= SCICR2_RIE_MASK;      /* enable RX complete interrupt         */
  _pSCI->scicr3.byte |= SCICR3_FEIE_MASK;     /* Enable Frame Error interrupt         */
  _pSCI->scibdh.byte |= SCIBDH_LBKDIE_MASK;        /* enable LIN Break Detection interrupt */
  #endif /* End (SCI_VERSION == SCI_V4)*/
